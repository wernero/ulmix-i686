...
static int ext2_probe(file_t *device, mbr_entry_t part);
static int ext2_mount(fnode_t *mountpoint, file_t *device, mbr_entry_t part);
...
static int ext2_probe(file_t *device, mbr_entry_t part)
{
    ...
    device->seek(device->drv_struct, part.start_sector + (superblock_offset / device->io_size), SEEK_SET);
    device->read(device->drv_struct, (char*)superblock, superblock_size / device->io_size);
